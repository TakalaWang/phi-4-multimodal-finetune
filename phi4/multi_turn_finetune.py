...
def finetune(args, stage):
    stage_name = stage.get("name")
    base_model = stage.get("base_model")
    conversations = stage.get("conversations", [])
    predict = stage.get("predict")
    audios = stage.get("audios")
    
    print(f"=== 處理階段：{stage_name} ===")
    print(f"起始模型：{base_model}")
    ...
    if accelerator.is_main_process:
        ...
        if args.push_to_hub:
            processor.push_to_hub(stage_name)
    ...
    print(f"訓練完成，模型已儲存至 Hugging Face: {stage_name}\n")
